...
function StaffSection() {
  ...
  const [pendingStaffRows, setPendingStaffRows] = useState([]);
  ...
  async function doSaveStaff(rowsToSave, mergeDecisions = {}) {
    // Update local state
    const updated = [...staff];
    ...
    try {
      const resp = await fetch('/api/staff', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rows: rowsToSave, mergeDecisions }),
      });
      const data = await resp.json();
      if (data.needsReview) {
        // Server found duplicates
        setDupList(data.duplicates.map(d => ({ ...d, resolved: null })));
        setPendingStaffRows(rowsToSave);
  ...
  async function save() {
    await doSaveStaff([...reviewRows]);
  }

  async function resolveStaffDuplicates() {
    const decisions = {};
    for (const d of dupList) {
      decisions[`${d.newName}||${d.existingName}`] = d.resolved;
    }
    setShowDupModal(false);
    setDupList([]);
    await doSaveStaff(pendingStaffRows, decisions);
    setPendingStaffRows([]);
  }
    ...
    <div>
      ...
      {showDupModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 24, maxWidth: 480, width: "100%", maxHeight: "80vh", overflowY: "auto" }}>
            <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 6 }}>⚠️ 동일 인물 확인</div>
            <div style={{ fontSize: 13, color: "#888", marginBottom: 20 }}>이름이 비슷한 Staff가 있어요. 같은 사람인지 확인해주세요.</div>
            {dupList.map((d, i) => (
              <div key={i} style={{ border: "1px solid #eee", borderRadius: 12, padding: 14, marginBottom: 12, background: "#fafafa" }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
                  <span style={{ background: "#E8F8F0", color: "#0F6E56", padding: "4px 10px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>{d.existingName}</span>
                  <span style={{ fontSize: 13, color: "#aaa" }}>vs</span>
                  <span style={{ background: "#FFF8E7", color: "#854F0B", padding: "4px 10px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>{d.newName}</span>
                </div>
                <div style={{ fontSize: 12, color: "#888", marginBottom: 12 }}>같은 사람인가요?</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setDupList(prev => prev.map((x,idx) => idx===i?{...x,resolved:true}:x))}
                    style={{ flex: 1, padding: "8px 0", borderRadius: 10, border: "none", fontSize: 13, fontWeight: 600,
                      background: d.resolved===true?"#1a1a1a":"#f0f0f0", color: d.resolved===true?"#fff":"#333", cursor: "pointer" }}>
                    ✓ 같은 사람 (합치기)
                  </button>
                  <button onClick={() => setDupList(prev => prev.map((x,idx) => idx===i?{...x,resolved:false}:x))}
                    style={{ flex: 1, padding: "8px 0", borderRadius: 10, border: "none", fontSize: 13, fontWeight: 600,
                      background: d.resolved===false?"#1a1a1a":"#f0f0f0", color: d.resolved===false?"#fff":"#333", cursor: "pointer" }}>
                    ✗ 다른 사람 (따로)
                  </button>
                </div>
              </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button onClick={() => { setShowDupModal(false); setDupList([]); setPendingStaffRows([]); }}
                style={{ flex: 1, padding: "10px 0", borderRadius: 12, border: "1px solid #ddd", background: "#fff", fontSize: 13, cursor: "pointer" }}>취소</button>
              <button onClick={resolveStaffDuplicates} disabled={dupList.some(d => d.resolved === null)}
                style={{ flex: 2, padding: "10px 0", borderRadius: 12, border: "none", fontSize: 13, fontWeight: 600,
                  background: dupList.some(d=>d.resolved===null)?"#ccc":"#1a1a1a", color: "#fff",
                  cursor: dupList.some(d=>d.resolved===null)?"not-allowed":"pointer" }}>
                {dupList.some(d=>d.resolved===null) ? "모두 선택해주세요" : "확인 완료 → 저장"}
              </button>
            </div>
          </div>
        </div>
      ...
      {sheetStatus && (
        <div style={{ padding: "10px 14px", borderRadius: 10, marginBottom: 12, fontSize: 13, fontWeight: 500,
          background: sheetStatus==='saved'?'#E8F8F0':sheetStatus==='saving'?'#FFF8E7':'#FEF2F2',
          color: sheetStatus==='saved'?'#0F6E56':sheetStatus==='saving'?'#854F0B':'#A32D2D' }}>
          {sheetStatus==='saving'?'⏳ Google Sheets에 저장 중...':sheetStatus==='saved'?'✅ Site Staff 탭에 저장됐어요!':'⚠️ Sheets 저장 실패'}
        </div>
      )}
